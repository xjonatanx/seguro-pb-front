...
async function enterForm() {
  if (!loginForm.rut) {
    return (error.value =
      "Por favor, ingresa tu RUT para validar tu identidad.");
  }
  ...
  try {
    const res: any = await $fetch(
      "https://pybingenieriachile.cl/api-seguro/api/auth/login",
      {
        method: "POST",
    ...
    error.value =
      "Error de conexión con el servidor de P&B. Intenta más tarde.";
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div
    class="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans text-slate-900"
  >
    <div
    ...
    >
      <header class="text-center mb-4">
        <h1
          class="text-4xl font-black text-slate-800 tracking-tighter italic uppercase"
        >
          P&B Ingeniería
        </h1>
        <p
          class="text-blue-900 font-bold uppercase text-[10px] tracking-[0.2em] mt-2 opacity-80"
        >
      ...
      <div
        class="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-xl shadow-sm mb-8"
      >
        ...
        <p class="mt-2 text-[13px] text-amber-700 leading-relaxed font-medium">
          Este beneficio es
          <span class="font-bold underline">exclusivo</span> para personal que
          ha realizado la transición a
          <span class="font-bold">Contrato por Obra</span>. Si usted se
          encuentra bajo otra modalidad contractual, su solicitud será rechazada
      ...
      <div class="space-y-6">
        <div>
          <label
            class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 block mb-2"
          >
            RUT del Trabajador
          </label>
          <input
        ...
        <div
          v-if="error"
          class="bg-red-50 border-l-4 border-red-500 p-4 rounded-xl animate-in fade-in duration-300"
        >
          <p class="text-red-700 text-xs font-black uppercase tracking-tight">
            {{ error }}
          </p>
        </div>
        ...
        <p
          class="text-center text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-6 italic"
        >
